<template>
    <div class="cardupd" style="position:relative; min-height: 100vh; margin: 0px; background: linear-gradient(to bottom, #517790 0%, #3B3303 100%);">
        <div align="middle">
            <p class="alert alert-danger" v-if="error">{{ error }}</p>
        </div>
        <div align="right" style="color: white;">
            <router-link :to="`/home`">
                <button style="height: 30px; width: 90px; border: 1px solid black; color: black">Home</button>
            </router-link> | 
            <button @click="exportLists" style="height: 30px; width: 90px; border: 1px solid black; color: black">Export</button> |
            <router-link :to="`/summary`">
                <button style="height: 30px; width: 90px; border: 1px solid black; color: black">Summary</button>
            </router-link> |
            <router-link :to="`/`">
                <button @click="clean" style="height: 30px; width: 90px; border: 1px solid black; color: black">Logout</button>
            </router-link>
        </div>
        <div align="middle" style="left:460px; top: 40px; position: absolute; margin: 60px; box-shadow:10px 10px 15px 15px #FABF05; background: wheat">
        <h3 align="center" style="color: black">Update Card</h3>
        <div class="container" style="font-size: 1.5rem; display: inline-block; margin: 10px 50px 20px;">
            <div>
                <label style="color: black">List</label>
                <select class="form-select" v-model="nlid">
                <option v-for="option in this.lists" :key="option.list_id" :value="option.list_id">
                {{ option.list_name }}
                </option>
            </select>
            </div>
            <div class="row" style="position: relative; margin: 50px 48px 50px;">
                <label style="color: black"><strong>Card Name </strong></label>
                <input type="text" v-model="ncname" name="ncname" placeholder="Please enter card name" style="height: 20px" />
            </div>
            <div class="row" style="position: relative; margin: 50px 48px 50px;">
                <label style="color: black"><strong>Card Content </strong></label>
                <input type="text" v-model="nccont" name="nccont" placeholder="Please enter card content" style="height: 20px"/>
            </div>
            <div class="row" style="position: relative; margin: 50px 48px 50px;">
                <label style="color: black"><strong>Deadline Date </strong></label>
                <input type="date" v-model="ndd" name="ndd" style="height: 20px"/>
            </div>
            <div class="row" style="position: relative; margin: 50px 48px 50px;">
                <label style="color: black"><strong>Mark as complete </strong></label>
                <input type="checkbox" v-model="ncompd" name = "ncompd" style="height: 20px"/>
            </div>
        </div>
        <div class="row" style="margin: 10px 48px 20px;"></div>
        <router-link :to="`/home`">
            <button style="padding: 14px 40px; background-color: white; color: black; border: 2px solid black; border-radius: 50%; margin: 10px 48px 20px;" type="submit" @click="updateCard">Update Card</button>
        </router-link>
        </div>
  </div>
</template>

<script>
    export default {
        name: "CardUpdating",
        data() {
            return{
                olid: null,
                nlid: null,
                cid: null,
                cname: null,
                ccont: null,
                dd: null,
                email: null,
                auth_token: null,
                error: "",
                lists: [],
                compd: null,
                ncname: null,
                nccont: null,
                ndd: null,
                ncompd: null
            };
        },
        async mounted(){
            this.email = sessionStorage.getItem("email");
            this.auth_token = sessionStorage.getItem("auth-token");
            this.olid = localStorage.getItem("lid")
            this.cid = localStorage.getItem("cid")
            this.cname = localStorage.getItem("cname")
            this.ccont = localStorage.getItem("ccont")
            this.dd = localStorage.getItem("dd")
            this.compd = localStorage.getItem("compd")
            this.lists = localStorage.getItem("lists")
            return fetch(`http://127.0.0.1:5000/api/list/${this.email}`,{
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                    "Authentication-Token": `${this.auth_token}`
                }
            }).then((response) => response.json())
            .then((d1) => {
                this.lists = d1
                console.log(d1)
                localStorage.setItem("lists", this.lists)
            }).catch((error) => console.log("1st", error))
        },
        async created() {
            this.email = sessionStorage.getItem("email");
            this.auth_token = sessionStorage.getItem("auth-token");
            if(!this.auth_token){
                alert("Login to view your Dashboard")
                this.$router.push("/")
            }
            this.olid = localStorage.getItem("lid")
            this.cid = localStorage.getItem("cid")
            this.cname = localStorage.getItem("cname")
            this.ccont = localStorage.getItem("ccont")
            this.dd = localStorage.getItem("dd")
            this.compd = localStorage.getItem("compd")
            this.lists = localStorage.getItem("lists")
            return fetch(`http://127.0.0.1:5000/api/list/${this.email}`,{
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                    "Authentication-Token": `${this.auth_token}`
                }
            }).then((response) => response.json())
            .then((d1) => {
                this.lists = d1
                console.log(d1)
                localStorage.setItem("lists", this.lists)
            }).catch((error) => console.log("1st", error))
        },
        methods: {
            async clean() {
                sessionStorage.clear()
                localStorage.clear()
            },
            async updateCard() {
                if(!this.ncname) {
                    this.ncname = this.cname
                }
                if(!this.nccont) {
                    this.nccont = this.ccont
                }
                if(!this.ndd) {
                    this.ndd = this.dd
                }
                if(!this.ncompd) {
                    this.ncompd = this.compd
                }
                if(this.olid!=this.nlid && this.nlid) {
                    this.olid=this.nlid
                }
                try {
                    fetch(`http://127.0.0.1:5000/api/card/${this.email}/${this.olid}/${this.cid}`, {
                        method: "PUT",
                        headers: {
                            "Content-Type": "application/json;charset=utf-8",
                            "Authentication-Token": `${this.auth_token}`,
                        },
                        body: JSON.stringify({
                            email: this.email,      
                            card_name : this.ncname,
                            card_content : this.nccont,
                            deadline_date : this.ndd,
                            completion_date : this.ncompd
                        }),
                    })
                    .then((resp) =>  resp.json())
                    .then((d2) => {
                        const response = d2;
                        if (response.message) {
                            this.error = response.message;
                            console.log(response.message);
                        } else {
                            this.c = d2;
                            console.log(this.c);
                            console.log("Card Updated");
                            this.$router.go("home");
                            console.log("its dashboard returns");
                        }
                    })
                    .catch((error) => {
                        console.log("1st",error);
                    });
                } catch (error) {
                    console.log("2nd",error);
                }
            }
        }

    }
</script>
